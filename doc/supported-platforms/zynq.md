...
- \ref sect_zynq_emacps
  - openPOWERLINK runs on Linux which is running on the ARM processing system (PS)
  of the SoC processor and this design uses the on-board MAC.
...
# openPOWERLINK MN on Zynq Emacps design {#sect_zynq_emacps}
...
Refer [this](\ref sect_software_requirements) section for the list of software packages and their
dependencies to run the openPOWERLINK Linux MN demo on Zynq ZC702.
...
(Enable by pressing space key, where [M] denotes that enabled devices are included as modules)
  * Disable other devices except the three provided by Xilinx (Refer the below image)
  * Click Save -> ok -> Exit. Then click on Exit to come out of the Kernel configuration window.

![](\ref zynq/reconfigure_linux_kernel.png)
...
![](\ref zynq/config_stack_emacps.png)
...
![](\ref zynq/config_driver_emacps.png)
...
![](\ref zynq/config_application_emacps.png)
