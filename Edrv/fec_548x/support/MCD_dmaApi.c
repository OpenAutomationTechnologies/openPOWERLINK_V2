...
/*
 * MCD_stopBuff is a generic buffer descriptor to which single-buffer
 * DMAs from the parameter list are chained.  This allows the same
 * loops to be used chained and single-buffer DMAs. This is declared
 * in modelTaskTable
 *
 * Usage TBD
 * extern MCD_bufDesc MCD_stopBuff;
 */

/*
 * Stop Buffer descriptor for relocation
 * Usage TBD
 */
MCD_bufDesc *MCD_relocStopBuff;
...
int MCD_initDma (dmaRegs *dmaBarAddr, void *taskTableDest, u32 flags)
{
   ...
   /* Set up pausing mechanism to inactive state: */
    ...
    MCD_dmaBar->debugControl = DEBUGCRDISABLEVALUE;
    MCD_dmaBar->debugStatus = DEBUGKILLALLSTATUS;

    /* enable or disable commbus prefetch, really need an ifdef or
       something to keep from trying to set this in the 8220 */
    if ((flags & MCD_COMM_PREFETCH_EN) != 0)
        MCD_dmaBar->ptd_ivr.ptd_cntl &= ~PTD_CNTL_COMM_PREFETCH;
    else
        MCD_dmaBar->ptd_ivr.ptd_cntl |= PTD_CNTL_COMM_PREFETCH;
...
int MCD_dmaStatus (int channel)
{
    ...
    else /* something is running */
    {
        ...
        if (   MCD_chStatus[channel] == MCD_RUNNING
            || MCD_chStatus[channel] == MCD_IDLE)
        {
            MCD_dmaBar->ptdDebug = REGINITIATOR;
...
printf("startDma:Setting up params\n");
   ...
   /* Set us up for task-wise priority.  We don't technically need to do this on every start, but
    ...
    MCD_dmaBar->ptd_ivr.ptd_cntl |= (short) 0x8000;
   ...
   /* Write the initiator field in the TCR, and also set the initiator-hold
    ...
    MCD_dmaBar->taskControl[channel] =
        (initiator << 8) | TASK_CONTROL_HIPRITSKEN | TASK_CONTROL_HLDINITNUM;
    ...
    {
        /* some of this is needed for progress query calculation, may be able to get
        rid of some of it TBD*/
        MCD_singleBufDescs[channel].srcAddr = srcAddr;
        MCD_singleBufDescs[channel].destAddr = destAddr;
        MCD_singleBufDescs[channel].lastDestAddr = destAddr;  /* definitely not its final value */
        MCD_singleBufDescs[channel].dmaSize = dmaSize;
        MCD_singleBufDescs[channel].flags = (flags & ~MCD_SINGLE_DMA ) | MCD_BUF_READY | MCD_CRC_RESTART;
        MCD_singleBufDescs[channel].csumResult = 0;
        MCD_singleBufDescs[channel].next = MCD_relocStopBuff;
        ...
        ((volatile int *)MCD_taskTable[channel].contextSaveSpace)[CURRBD + CSAVE_OFFSET] =
                                             (u32) &(MCD_singleBufDescs[channel]);

        if( funcDesc == NOEU1 || funcDesc == NOEU2)
        {/* Use the 13th variant */
           /* TDTStart and TDTEnd */
           ...
           MCD_startDmaSingleNoEu(srcAddr, srcIncr, destAddr, destIncr, dmaSize, (dmaSize-xferSize),
                                xferSizeIncr, flags, (int *)&(MCD_singleBufDescs[channel]), cSave,
                                MCD_taskTable, channel);
        }
        else
        {/* Use the 11th variant */
           /* TDTStart and TDTEnd */
           ...
           MCD_startDmaSingleEu(srcAddr, srcIncr, destAddr, destIncr, dmaSize, (dmaSize-xferSize),
                              xferSizeIncr, flags, (int *)&(MCD_singleBufDescs[channel]), cSave,
                              MCD_taskTable, channel);
        }
    }
 ...
 *  DMA-destination address not (or not yet) used in the DMA.
 *
...
static void MCD_resmActions (int channel)
{
   ...
   MCD_dmaBar->ptdDebug = REGINITIATOR; /* This register is selected to know
...
int MCD_continDma (int channel)
{
    ...
    MCD_dmaBar->taskControl[channel] |= TASK_CONTROL_EN;
    MCD_chStatus[channel] = MCD_RUNNING;
...
int MCD_pauseDma (int channel)
{
    ...
    if (MCD_dmaBar->taskControl[channel] & TASK_CONTROL_EN)
    {
        MCD_dmaBar->debugComp1 = channel;
        MCD_dmaBar->debugControl = DEBUGCRENABLEVALUE | (1 << (channel + 16));
 ...
 *
 * Notes:
 *  TBD - have to fetch from the context save space?
 */
int MCD_csumQuery (int channel, u32 *csum)
{
#if 0
    return MCD_relocBuffDesc[channel].csumResult;
#else
    return(MCD_ERROR);
#endif
}
 ...
 */
char MCD_versionString[] = "Multi-channel DMA API Alpha v0.2 (2004-03-25)";
#define MCD_REV_MAJOR   0x00
#define MCD_REV_MINOR   0x02
